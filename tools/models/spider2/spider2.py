...
COXA = 0
FEMUR = 1
#TIBIA = 2 removing tibia 02/17/19 KB

class spider2(dict):#changed name from lizard3 to spider2
    ...
    L_FEMUR = 50    # MM distance from femur servo to tibia servo --this becomes distance from femur servo to foot
    ...
    def __init__(self, opt=4, debug=False, gaitGen = None):
        ...
        self.vars_layout = [["Leg", 0, 1, "leg.jpg"], ["Offsets",3,4,5,"body.jpg"], ["COG Offsets",6,7,""]] # removed 2
        self.vars = { 0:["Coxa(mm)",50],1:["Femur(mm)",50],2:["X(mm)",50],3:["Y(mm)",50],4:["Mid-Y(mm)",50],5:["X(mm)",0],6:["Y(mm)",0] } #removed 2:["Tibia(mm)",50], and changed numbers accordingly

        # Column 3 = Servo Values, need both specification of layout and data.
        self.servo_layout = ["LF Coxa","RF Coxa","LF Femur","RF Femur","","","LM Coxa","RM Coxa","LM Femur","RM Femur","","","LR Coxa","RR Coxa","LR Femur","RR Femur",] #removed "LF Tibia","RF Tibia", "LM Tibia","RM Tibia", "LR Tibia","RR Tibia"
        self.servos = { "RF Coxa":1, "RF Femur":7, "LF Coxa":6,"LF Femur": 12,"RR Coxa":2, "RR Femur":8,  "LR Coxa":4, "LR Femur":10,  "RM Coxa":2,"RM Femur":8,"LM Coxa":5,"LM Femur":11 }
            # removed "RF Tibia":5,"LF Tibia":6,"RR Tibia":11,"LR Tibia":12,"RM Tibia":17, "LM Tibia":18 from above 02/17/19 KB
            #also changing servo id numbers to match
        ...
        self.order = {"RF_GAIT":0,"LR_GAIT":2,"LF_GAIT":4,"RR_GAIT":6}
            #removed one zero from each of the gait arrays above
    ...
    def config(self, opt, dims=None, servos=None, resolutions=None):
        ...
        if dims != None:
            self.L_COXA = dims[0]
            self.L_FEMUR = dims[1]
            #self.L_TIBIA = dims[2] commented out 02/17/19 KB
            self.X_COXA = dims[3]
            self.Y_COXA = dims[4]
            self.Y_MID = dims[5]
            # cogs?
        ...
        if servos != None:
            self.servos["LF Coxa"] = servos[6]
            self.servos["RF Coxa"] = servos[1]
            self.servos["LF Femur"] = servos[12]
            self.servos["RF Femur"] = servos[7]
            #self.servos["LF Tibia"] = servos[4] commented out 02/17/19 KB
            #self.servos["RF Tibia"] = servos[5] commented out 02/17/19 KB

            self.servos["LM Coxa"] = servos[5]
            self.servos["RM Coxa"] = servos[2]
            self.servos["LM Femur"] = servos[11]
            self.servos["RM Femur"] = servos[8]
            #self.servos["LM Tibia"] = servos[10] commented out 02/17/19 KB
            #self.servos["RM Tibia"] = servos[11] commented out 02/17/19 KB

            self.servos["LR Coxa"] = servos[4]
            self.servos["RR Coxa"] = servos[3]
            self.servos["LR Femur"] = servos[10]
            self.servos["RR Femur"] = servos[9]
            #self.servos["LR Tibia"] = servos[16] commented out 02/17/19 KB
            #self.servos["RR Tibia"] = servos[17] commented out 02/17/19 KB
    ...
    def legIK(self, X, Y, Z, resolution):
        ...
        ans = [0,0,0]    # (coxa, femur, tibia) #removed one ,0 to take out femur  02/17/19 KB
        ...
        try:
            ...
            #ans[2] = radToServo(acos(d1/float(d2))-1.57, resolution) commented out 02/17/19 KB
        except:
            if self.debug:
                print "LegIK FAILED"
